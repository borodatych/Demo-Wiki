...
class Emitter
{
	...
	protected $_blocktags = // could be replaced or changed by client
		...
		(
			...
			'table', 'thead', 'tbody', 'tr', 'td', 'th', 'tfoot', 'caption', 'colgroup','col', # table
...
*/
	...
	*/
	protected function prepare_image_node($node)
	{
		...
		// caption, or...
		if ($node->caption) {
			$node->decoration->attributes['alt'] = substr($this->char_filter(strip_tags($node->caption),$node),0,60) . '...';
