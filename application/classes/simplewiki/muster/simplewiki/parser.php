...
/**@{ @name Data properties */
	...
	protected $_argchars = '[^()]'; // no parenthesis; for any char, use '(?s:.)' (any char including newline)
...
//		$rules->char =  '(?P<char> ([\\w\\s]+$|. ))'; //faster, but misses raw url's, twice as slow as markup filter
		...
		// positive lookbehind for ~ catches escaped vertical bars for inclusion in extraction
		$rules->cell = '
...
*/
	public function prepare($markup)
	{
		$this->_metadata = NULL;
		$this->_raw = $markup;
        $this->_root = new DocNode(DocNode::DOCUMENT); # 'document' is the top level node
...
*/
	protected function preprocess_raw_markup($raw)
	{
		...
		# remove comments
//		$raw = preg_replace('/\n```.*$/m','',$raw);
		$raw = preg_replace('/```.*$/m','',$raw); // allow comment starting anywhere in line
		# remove line continuations
		$raw = preg_replace('/\n``/','',$raw);
...
*/
    protected function _char_node($preg_groups) // can create text leaf node
	{
        ...
        if (is_null($this->_leaftextnode))
            $this->_leaftextnode = new DocNode(DocNode::TEXT, $this->_curnode);
...
*/
    protected function _blankline_node($preg_groups)
	{
        ...
        $this->_curnode = $this->get_selected_ancestor($this->_curnode, 
			array(DocNode::DOCUMENT,DocNode::BLOCKDEF));
	}
    ...
    protected function _paragraph_node($preg_groups) // can create paragraph for new text
	{
        ...
        if (in_array($this->_curnode->type, 
			array(
				DocNode::TABLE, 
				DocNode::TABLE_ROW, 
				DocNode::UNORDERED_LIST, 
				DocNode::ORDERED_LIST,
				DocNode::DEF_LIST))) // text cannot exist in these blocks
		{
            $this->_curnode = $this->get_selected_ancestor($this->_curnode,
                array(DocNode::DOCUMENT,DocNode::BLOCKDEF));
		}
        if (in_array($this->_curnode->type, array(DocNode::DOCUMENT,DocNode::BLOCKDEF)))
		{
            $node = $this->_curnode = new DocNode(DocNode::PARAGRAPH, $this->_curnode);
...
*/
    protected function _heading_node($preg_groups)
	{
        ...
        $node = new DocNode(DocNode::HEADING,$this->_curnode);
...
*/
	...
	{
		# emphasis
        if ($this->_curnode->type != DocNode::EMPHASIS)
            $this->_curnode = new DocNode(DocNode::EMPHASIS, $this->_curnode);
        else
		{
...
*/
	...
	{
		# strong
        if ($this->_curnode->type != DocNode::STRONG)
            $this->_curnode = new DocNode(DocNode::STRONG, $this->_curnode);
        else
		{
			if (!empty($this->_curnode->parent))
				$this->_curnode = $this->_curnode->parent;
...
*/
	...
	{
		# line break
        new DocNode(DocNode::LINEBREAK, $this->_curnode);
        $this->_leaftextnode = NULL;
	}
...
*/
    protected function _horizontalrule_node($preg_groups)
	{
        $this->_curnode = $this->get_selected_ancestor($this->_curnode, 
			array(DocNode::DOCUMENT,DocNode::BLOCKDEF));
        new DocNode(DocNode::HORIZONTALRULE, $this->_curnode);
	}
...
*/
    protected function _link_node($preg_groups)
	{
		...
		$node =  new DocNode(DocNode::LINK, $this->_curnode);
		$node->target = $target;
		if ($decorator) $this->set_node_decoration($node,$decorator);
...
*/
	protected function _image_node($preg_groups)
	{
        ...
        $node = new DocNode(DocNode::IMAGE, $this->_curnode);
		$node->target = $target;
		if ($decorator) $this->set_node_decoration($node,$decorator);
		if ($title != '') $node->title = $title;
   ...
   protected function _item_node($preg_groups)
	{
        ...
        if ($bullet{0} == '#')
            $listtype = DocNode::ORDERED_LIST;
        else
            $listtype = DocNode::UNORDERED_LIST;
		...
		(
			...
			(
				in_array($candidate_node->type, array(DocNode::ORDERED_LIST, DocNode::UNORDERED_LIST)) 
				and $candidate_node->level == $level
			)
			and ! // ... and this is not a block ...
			(
				in_array($candidate_node->type, array(DocNode::DOCUMENT,DocNode::BLOCKDEF))
			)
		) // ... so keep looking.
		{
        ...
        else # ... or create a new level of list
		{
            ...
            $this->_curnode = $this->get_selected_ancestor($this->_curnode,
                array(DocNode::LIST_ITEM, DocNode::DOCUMENT,DocNode::BLOCKDEF));
			// create the list
            $listnode = $this->_curnode = new DocNode($listtype, $this->_curnode);
			if ($listdecorator) $this->set_node_decoration($listnode,$listdecorator);
            $listnode->level = $level;
		}
		# now add the list item to the list
        $itemnode = $this->_curnode = new DocNode(DocNode::LIST_ITEM, $this->_curnode);
...
*/
    protected function _defitem_node($preg_groups)
	{
		...
		(
			...
			and ! // this is a not a list of the same level...
			(
				($candidate_node->type == DocNode::DEF_LIST) 
				and $candidate_node->level == $level
			)
			and ! // ... and this is not a block ...
			(
        ...
        else # ... or create a new level of list
		{
			// get the nearest ancestor candidate for creating a new list
            $this->_curnode = $this->get_selected_ancestor($this->_curnode,
                array(DocNode::DEF_DESC, DocNode::DOCUMENT,DocNode::BLOCKDEF));
			// create the list
            $listnode = $this->_curnode = new DocNode($listtype, $this->_curnode);
			if ($listdecorator) $this->set_node_decoration($listnode,$listdecorator);
            $listnode->level = $level;
		...
		if ($term)
		{
			...
			$termnode = $this->_curnode = new DocNode(DocNode::DEF_TERM, $this->_curnode);
		...
		$descnode = $this->_curnode = new DocNode(DocNode::DEF_DESC, $this->_curnode);
...
*/
	protected function _table_node($preg_groups)
	{
		...
		$this->_curnode = $this->get_selected_ancestor($this->_curnode, 
			array(DocNode::TABLE,DocNode::DOCUMENT,DocNode::BLOCKDEF));
		# create new table node if necessary
		if ($this->_curnode->type != DocNode::TABLE)
			$this->_curnode = new DocNode(DocNode::TABLE, $this->_curnode);
		...
		$row_node = $this->_curnode = new DocNode(DocNode::TABLE_ROW, $tablenode);
		...
		foreach ($cell_matches as $cell_groups) {
			...
			if ($cellhead) {
				...
				$cell_node = $this->_curnode = new DocNode(DocNode::TABLE_HEADCELL, $row_node);
			} else {
				$cell_node = $this->_curnode = new DocNode(DocNode::TABLE_CELL, $row_node);
			}
...
*/
	protected function _span_node($preg_groups)
	{
		...
		{
            $node = $this->_curnode = new DocNode(DocNode::SPAN, $this->_curnode);
			$this->set_node_decoration($node,$decorator);
			$this->_leaftextnode = NULL;
		}
        elseif ($this->_curnode->type == DocNode::SPAN) // closing existing span
		{
			if (!empty($this->_curnode->parent))
			{
				$this->_curnode = $this->_curnode->parent;
...
*/
    protected function _blockdivider_node($preg_groups)
	{
        ...
        $this->_curnode = $this->get_selected_ancestor($this->_curnode, 
			array(DocNode::DOCUMENT,DocNode::BLOCKDEF));
        $node = new DocNode(DocNode::BLOCKDIVIDER, $this->_curnode);
		if ($decorator) $this->set_node_decoration($node,$decorator);
	}
	#============================[ preformatted text ]=================================#
...
*/
    protected function _code_node($preg_groups)
	{
        ...
        $node = new DocNode(DocNode::CODE, $this->_curnode);
...
*/
    protected function _pre_node($preg_groups)
	{
        ...
        $node = new DocNode(DocNode::PREFORMATTED, $this->_curnode);
...
*/
	protected function _blockdef_node($preg_groups)
	{
        ...
        $container = $this->_curnode = $this->get_selected_ancestor($this->_curnode,
            array(DocNode::DOCUMENT,
				DocNode::BLOCKDEF,
				DocNode::LIST_ITEM,
				DocNode::DEF_DESC));
        $node = $this->_curnode = new DocNode(DocNode::BLOCKDEF, $container);
		$node->blocktag = $name;
		if ($decorator) $this->set_node_decoration($node,$decorator);
...
*/
	protected function _macro_node($preg_groups)
	{
        ...
        $node = new DocNode(DocNode::MACRO, $container);
		$node->macroname = $name;
...
*/
	protected function _blockmacro_node($preg_groups)
	{
        ...
        $container = $this->_curnode = $this->get_selected_ancestor($this->_curnode, 
			array(DocNode::DOCUMENT,DocNode::BLOCKDEF)); // different from macro
        $node = new DocNode(DocNode::MACRO, $this->_curnode);
		$node->macroname = $name;
        if ($decorator) $this->set_node_decoration($node,$decorator);
		if ($text)
		{
